...
class TestDatabaseRepositoryGetLyrics(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # Create temp tables
        self.cursor.execute("CREATE TEMPORARY TABLE tmp_lyrics SELECT * FROM lyrics WHERE pl LIKE '%auto%'")
        
        self.dbrepo = DatabaseRepository(db=self.db)
        return super().setUp()

    def test_getting_lyrics(self):
        lyrics = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.SHORTESTS, table_name="tmp_lyrics")
    ...
    def test_soring_lyrics(self):
        lyrics_shortests = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.SHORTESTS, table_name="tmp_lyrics")
        lyrics_longests = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.LONGESTS, table_name="tmp_lyrics")
        lyrics_bests = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.BEST_MATCH, table_name="tmp_lyrics")
    ...
    def test_getting_only_specific_type(self):
        only_movies_lyrics = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.LONGESTS, only_movies=True, table_name="tmp_lyrics")
        only_series_lyrics = self.dbrepo.get_lyrics("auto", "pl", "en", SortingMode.LONGESTS, only_movies=False, table_name="tmp_lyrics")
